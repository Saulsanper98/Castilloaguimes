...
function PlayerAvatars({ total, occupied }: { total: number; occupied: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="text-[#f5f5f0]/40 text-xs ml-2">
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="pt-20 bg-[#111111] border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <button className="flex items-center gap-2 bg-[#3a7d44] hover:bg-[#4a9d54] text-white font-bold px-5 py-3 rounded-xl text-sm transition-all shrink-0">
              <Plus size={16} />
              Crear partido
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {/* Filters */}
        <div className="bg-[#111111] border border-white/10 rounded-2xl p-4 mb-8 flex flex-col sm:flex-row gap-3">
          <input
            type="date"
            value={filterDate}
            onChange={(e) => setFilterDate(e.target.value)}
            className="flex-1 bg-[#1a1a1a] border border-white/10 text-[#f5f5f0] text-sm rounded-xl px-4 py-2.5 outline-none focus:border-[#3a7d44]/60 transition-colors"
          />
          <select
            value={filterLevel}
          ...
          {(filterDate || filterLevel || filterHour) && (
            <button
              onClick={() => { setFilterDate(""); setFilterLevel(""); setFilterHour("") }}
              className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-[#f5f5f0]/60 text-sm rounded-xl border border-white/10 transition-colors"
            >
              Limpiar
            </button>
          )}
        ...
        {/* Count */}
        <p className="text-[#f5f5f0]/40 text-sm mb-5">
          {filtered.length} partido{filtered.length !== 1 ? "s" : ""} disponible{filtered.length !== 1 ? "s" : ""}
        </p>

        {/* Grid */}
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-[#f5f5f0]/30">
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-white/10 pt-4">
                    <span className="text-[#3a7d44] font-black text-lg">
                      {partido.precio}€
                      <span className="text-[#f5f5f0]/30 text-xs font-normal"> /jugador</span>
                    </span>
                    <button
                      disabled={isFull}
                      className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                        isFull
                          ? "bg-white/5 text-[#f5f5f0]/30 cursor-not-allowed"
